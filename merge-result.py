...
for group_name, group_files in groups.items():
    ...
    merged_df['merge-results'] = merged_df.apply(lambda row: 1 if sum([
        row['angles_prediction_results'] == 1,
        row['distance_prediction_results'] == 1,
        row['secondary-structure_prediction_results'] == 1
    ]) >= 2 else 0, axis=1)
